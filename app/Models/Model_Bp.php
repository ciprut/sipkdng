    ...
    public function simpanNSUP($post){
      if(session()->edit == ""){
		...
		public function BaseQuerySpp(){
			...
			$builder->select('S.UNITKEY, rtrim(S.KDSTATUS) as KDSTATUS, S.NOSPP, S.TGSPP, S.IDXSKO, S.KETOTOR, RR.NILAI');
			$builder->select('S.NOREG, S.NOKONTRAK, S.KEPERLUAN, rtrim(S.KEYBEND) as KEYBEND, S.KDP3, S.KD_BULAN,B.JNS_BEND,');
			$builder->select('S.PENOLAKAN, S.TGLVALID, S.IDXKODE,rtrim(S.IDXTTD) as IDXTTD,SK.TGLSKO,SK.NOSKO, \'\'  as KDKEGUNIT,');
      ...
      $builder->join('SPPDETR D','S.NOSPP = D.NOSPP and S.UNITKEY = D.UNITKEY', 'LEFT OUTER');
		...
		public function getSPP(){
			...
			$builder->select('a.*,CAST(a.TGSPP as datetime) as TGLSPP, b.KDUNIT, b.NMUNIT')->where('a.NOSPP',session()->nospp)->where('a.UNITKEY',session()->kdUnit);
      $builder->join('DAFTUNIT b','b.UNITKEY = a.UNITKEY AND b.KDLEVEL = "3"');
			return $builder->get()->getRow();
		}
