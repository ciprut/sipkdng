...
<form id='frmSPP'>
  <?php
    ...
    $form->addHidden(array("id"=>"kp3","value"=>""));
    ...
    if($spp->NOSPP != ''){
      ...
      $kep = $keperluan;
      $nospp = pjg((session()->noreg), 5)."/SPP-".strtoupper(session()->jnsSpp)."/".getKdSKPD($unit->KDUNIT)."/".$bendahara->JAB_BEND."/".session()->cur_thang;
    ...
    $form->addGroup(array("type"=>"textarea","id"=>"txtUntuk","label"=>"Untuk","readonly"=>"0","placeholder"=>"","value"=>""));
    ...
    $row = array(
      array("width"=>"12","type"=>"text","id"=>"txtNoKontrak","label"=>"No Kontrak","readonly"=>"0","value"=>"")
    );
    $form->addRow($row);
