<?php
  ...
  $form->addClear("10");
  $form->addButton(array("id"=>"btnTambah","icon"=>"plus","title"=>"Tambah KPA","color"=>"primary"));
...
<script>
  ...
  $("#btnTambah").click(function(){
    post_form("formKPA","nip=","Form Kuasa Pengguna Anggaran");
  });
  ...
  $('#tblPA').on("click",".hapus",function(){
    ...
    showModal({color:"danger",isi:"Yakin akan melanjutkan proses ini?"},function(){
      post_to_content("listKPA","hapusKPA","nip="+elm)
    });
  });
  $('#tblPA').on("click",".ubah",function(){
    elm = $(this).data("elm");
    post_form("formKPA","nip="+elm,"Form Edit KPA");
  });
